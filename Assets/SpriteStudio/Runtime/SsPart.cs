...
public class SsPart : IComparable<SsPart>
{
...
#if _USE_TRIANGLE_STRIP
		_mesh.SetTriangleStrip(_visible ? _triIndices : null, index);
#else
		_mesh.SetTriangles(_visible ? _triIndices : null, index);
#endif
		_subMeshIndex = index;
	}
	...
	Update()
	{
		int frame = (int)_mgr.AnimFrame;
		
		// priority 
		int nowPrio = (int)_res.Prio(frame);
		...
		if (_res.IsRoot)
		{
			_visible = !_res.Hide(frame);
		}
		else
		if (_res.Type == SsPartType.Normal)
		{
			bool nowVisible;
			if (_res.IsBeforeFirstKey(frame))
				nowVisible = false;
			else
			{
				if	(_parent != null
				&&	!_parent._res.IsRoot
				&&	(_res.InheritRate(SsKeyAttr.Hide) > 0.5f))
					nowVisible = _parent._visible;
				else
					nowVisible = !_res.Hide(frame);
			}
		...
		// vertex color
		if (_res.HasAttrFlags(SsKeyAttrFlags.PartsCol))
		{
			SsColorBlendKeyValue cbk = _res.PartsCol(frame);
		...
		if (_hasTransparency)
		{
			float nowAlpha = _res.Trans(frame);
			if (_parent != null && _res.Inherits(SsKeyAttr.Trans))
			{
				float parentAlpha;
		...
		// scale
		if (_res.HasAttrFlags(SsKeyAttrFlags.Scale))
		{
			var scale = new Vector3(_res.ScaleX(frame), _res.ScaleY(frame), 1f);
			if (scale != _scale)
			{
				_scale = scale;
		...
		if (_res.HasAttrFlags(SsKeyAttrFlags.Angle))
		{
			var ang = _res.Angle(frame);
		...
		// translate
		if (_res.HasAttrFlags(SsKeyAttrFlags.Pos))
		{
			var	pos = new Vector3(_res.PosX(frame), -_res.PosY(frame));
#if false
			if (_parent != null)
			{
			    if (_parent._flipH)
		...
		// UV animation
		if (_res.HasAttrFlags(SsKeyAttrFlags.ImageOffset))
		{
			int nowImgOfs = _res.ImageOffsetX(frame);
			if (nowImgOfs != _imgOfsX)
			{
				_imgOfsX = nowImgOfs;
				_mgr._uvChanged = true;
			}
			nowImgOfs = _res.ImageOffsetY(frame);
			...
			nowImgOfs = _res.ImageOffsetW(frame);
			...
			nowImgOfs = _res.ImageOffsetH(frame);
			...
			{
				// modify polygon size
				Vector2 size = _res.PicArea.WH();
				size.x += _imgOfsW;
				size.y += _imgOfsH;
				_orgVertices = _res.GetVertices(size);
				orgVertChanged = true;
			}
			if (_mgr._uvChanged)
			{
				_res.CalcUVs(_imgOfsX, _imgOfsY, _imgOfsW, _imgOfsH);
			}
		}
		
		// origin animation
		if (_res.HasAttrFlags(SsKeyAttrFlags.OriginOffset))
		{
			int nowOrgOfsX = -_res.OriginOffsetX(frame);
			if (nowOrgOfsX != _originOffset.x)
			{
				_originOffset.x = nowOrgOfsX;
				orgVertChanged = true;
			}
			int nowOrgOfsY = _res.OriginOffsetY(frame);
		...
		if (_res.HasAttrFlags(SsKeyAttrFlags.Vertex))
			orgVertChanged = true;
		...
		&&	_vertPositions != null)
		{
			for (int i = 0; i < _vertPositions.Length; ++i)
			{
				...
				if (_res.HasAttrFlags(SsKeyAttrFlags.Vertex))
				    _vertPositions[i] += _res.Vertex(frame).Vertex3(i);
				if (_res.HasAttrFlags(SsKeyAttrFlags.OriginOffset))
		...
		bool dontFlipCoord = _res.IsRoot ? false : _mgr._animation.hvFlipForImageOnly;
		...
		else
		{
			if (dontFlipCoord)
			{
				if (_parent != null  && _res.Inherits(SsKeyAttr.FlipH))
		...
		else
		{
			if (dontFlipCoord)
			{
				if (_parent != null  && _res.Inherits(SsKeyAttr.FlipV))
					if (!_parent._res.IsRoot)
		...
		// udpate uv indices
		if (_mgr._uvChanged
		&&	_res.UVs != null	// root part has no UVs
		&&	_res.UVs.Length == 4)
		{
			if (dontFlipCoord)
			{
				...
				if (nowFlipH) ++index;
				for (int i = 0; i < 4; ++i)
					_mgr._uvs[_vIndex + i] = _res.UVs[ index >= 0 ? _flippedUvIndices[index, i] : i ];
			}
			else
			{
				for (int i = 0; i < 4; ++i)
		...
		if (_mgr._vertChanged)
		{
			...
			if (_parent)
			{
				...
				if (!_res.Inherits(SsKeyAttr.ScaleX))
				{
					s.x /= _parent._scale.x;
					p.x /= _parent._scale.x;
				}
				if (!_res.Inherits(SsKeyAttr.ScaleY))
				{
	...
	_OnEvent(SsKeyAttr attr)
	{
		...
		int frame = 0;
		if (values.Length > 1)
			frame = (int)_mgr._animeFrame;
		
		if (frame < 0)
			frame = 0;
		else if (frame > _mgr._endAnimeFrame)
			frame = _mgr._endAnimeFrame;
		...
		if (key.Time != (int)_mgr._animeFrame)
			return;
		// OK
		switch (attr)
		{
