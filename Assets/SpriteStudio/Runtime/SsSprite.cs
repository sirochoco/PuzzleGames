...
//				Debug.Log("lost frame at " + Time.frameCount);
			...
			||	(int)_animeFrame > _endAnimeFrame) // 2012.02.05 cares about last frame
			{
				...
				{
					// stop
					if (_animeFrame < _startAnimeFrame) _animeFrame = _startAnimeFrame;
					else if (_animeFrame > _endAnimeFrame) _animeFrame = _endAnimeFrame;
				}
				else
				{
					// repeat
					if (_animeFrame < _startAnimeFrame)
					{
						// make overrun frame value during start~end.
						float overrun = (_animeFrame - _startAnimeFrame) % _playFrameLength;
						_animeFrame = _endAnimeFrame + 1 + overrun; // 2012.02.05 cares about last frame
					}
					else if (_animeFrame > _endAnimeFrame)
					{
						// make overrun frames during start~end.
						float overrun = (_animeFrame - _endAnimeFrame) % _playFrameLength;
