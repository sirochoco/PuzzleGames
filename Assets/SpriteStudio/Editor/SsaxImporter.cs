...
//		Debug.Log("SsAnime._Parse()");
		...
		{
			Debug.Log("EndFrame node is not found");
			return false;
		}
				...
				else
				{
					foreach (XmlNode key in keyList)
					{
						...
						case SsKeyValueType.User:
							{
								...
								{
									v.IsNum = true;
									v.Num = _ToUInt(numberNode.Value);
								}
						...
						if (bNeedCurve)
						{
							...
							v.Type = (SsInterpolationType)_ToUInt(key.Attributes["CurveType"].Value);
